...
/**
 * RecurringPanel
 * @param {string} type - 'ingreso' | 'gasto' | 'costo'
 * @param {array} categorias - available categories for this type
 * @param {array} metodosPago - available payment methods
 * @param {function} onUseTemplate - called with prefilled form data when user clicks "Usar"
 */
export default function RecurringPanel({ type, categorias, metodosPago, onUseTemplate, isReadOnly }) {
  const { templates, addTemplate, deleteTemplate, getPendingTemplates } = useApp();
  const [showForm, setShowForm] = useState(false);
  const [form, setForm] = useState({ categoria: "", descripcion: "", metodoPago: "", impuesto: 0, expectedDay: 5, notas: "" });
  ...
  const handleAddTemplate = () => {
    if (!form.categoria || !form.descripcion) {
      alert("Categoría y Descripción son obligatorias para crear la plantilla.");
      return;
    }
    addTemplate({ ...form, type });
    setForm({ categoria: "", descripcion: "", metodoPago: "", impuesto: 0, expectedDay: 5, notas: "" });
    setShowForm(false);
  };

  if (myTemplates.length === 0 && !showForm) {
    // Compact empty state - invite to create first template
    ...
    return (
      <div className="card" style={{ marginBottom: 20, borderStyle: "dashed", borderColor: a.border, textAlign: "center", padding: 18 }}>
        ...
        <button className="btn btn-ghost btn-sm" onClick={() => setShowForm(true)}>+ Crear primera plantilla recurrente</button>
      </div>
    );
  }
      ...
      {pending.length > 0 && (
        <div className="card" style={{ marginBottom: 14, borderColor: a.border, background: a.bg }}>
          <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 12 }}>
            ...
            <span style={{ fontWeight: 700, fontSize: 13, color: a.text }}>
              Pendientes de {MONTHS_ES[currentMonth - 1]} {currentYear} ({pending.length})
            </span>
          </div>
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            {pending.map(t => (
              <div key={t.id} style={{
                ...
                background: "var(--bg2)", border: `1px solid ${a.border}`, borderRadius: 8, padding: "8px 12px",
              }}>
                <div>
      ...
      <div className="card">
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 12 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            ...
            <span style={{ fontSize: 11, padding: "2px 8px", borderRadius: 20, background: a.bg, color: a.text, fontWeight: 700 }}>
              {myTemplates.length}
            </span>
          </div>
          {!isReadOnly && !showForm && (
            <button className="btn btn-ghost btn-sm" onClick={() => setShowForm(true)}>+ Nueva plantilla</button>
          )}
        </div>
          ...
          <div style={{ display: "flex", flexWrap: "wrap", gap: 8, marginBottom: showForm ? 14 : 0 }}>
            {myTemplates.map(t => (
              <div key={t.id} style={{
                ...
                background: "var(--bg3)", border: "1px solid var(--border)", borderRadius: 20, padding: "6px 6px 6px 12px",
              }}>
                ...
                {!isReadOnly && (
                  <>
                    <button className="btn btn-sm" style={{ background: a.text, color: "white", padding: "3px 10px", fontSize: 11 }}
                      onClick={() => onUseTemplate(t)}>
                      Usar
                    </button>
                    <button
                      onClick={() => confirm("¿Eliminar esta plantilla?") && deleteTemplate(t.id)}
          ...
          <div style={{ borderTop: myTemplates.length ? "1px solid var(--border)" : "none", paddingTop: myTemplates.length ? 14 : 0 }}>
            ...
            <div className="form-row" style={{ marginBottom: 10 }}>
              <div className="form-group">
                ...
                <input className="form-input" value={form.descripcion} placeholder="Ej: Alquiler Oficina"
                  onChange={e => setForm(p => ({ ...p, descripcion: e.target.value }))} />
              </div>
              <div className="form-group">
                <label className="form-label">Día esperado del mes</label>
                <input className="form-input" type="number" min="1" max="31" value={form.expectedDay}
                  onChange={e => setForm(p => ({ ...p, expectedDay: +e.target.value }))} />
              </div>
            </div>
            <div className="form-row" style={{ marginBottom: 12 }}>
              <div className="form-group">
                <label className="form-label">Impuesto % (opcional)</label>
                <input className="form-input" type="number" min="0" step="0.01" value={form.impuesto}
            ...
            <div style={{ display: "flex", gap: 8 }}>
              ...
              <button className="btn btn-sm" style={{ background: a.text, color: "white" }} onClick={handleAddTemplate}>
                Guardar Plantilla
              </button>
            </div>
          </div>
